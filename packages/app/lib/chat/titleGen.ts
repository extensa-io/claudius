...
/**
 * Fire-and-forget: title a new conversation with a cheap Haiku call and persist
 * it. It still routes through `assertCanInvoke` (invariant #3) and writes its own
 * usage_events row with purpose `title_gen`, but it does *not* consume the user's
 * daily message allowance — it is a system call, not a turn the user typed.
 *
 * The user's opening message alone is enough, and `assistantText` is optional on
 * purpose: titling used to wait for the first exchange to finish, which meant a
 * first turn that died mid-run (an abort, a platform timeout) left the
 * conversation stuck on "New chat" forever, with no second chance — only the
 * first turn is ever titled. Kicking this off as soon as the conversation row
 * exists makes the title independent of whether the reply survives.
 *
...
export async function generateTitle(params: {
  ...
  try {
    ...
    const response = await model.invoke([
      ...
      new HumanMessage(
        params.assistantText && params.assistantText.length > 0
